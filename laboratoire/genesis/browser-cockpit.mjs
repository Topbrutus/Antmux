...
const EXPECTED={title:'Genesis Vision Center — DEMO',banner:'DEMO / SYNTHETIC DATA',contract:'2.0.0-draft',mode:'DEMO',source:'SYNTHETIC',publication:'DEMO-GENESIS-V2-0001'};
...
async function verify(base,name,viewport){
  ...
  try{
    ...
    for(const selector of ['#rootStatus','#cycle','#nextTest','#trainingObservations','#scientificRule','#publicationGates']) assert(await page.locator(selector).count()===1,`${name}: section ${selector} absente`);
    assert((await text(page,'#scientificRule'))==='MESURE != INTERPRÉTATION',`${name}: règle scientifique affaiblie`);
    const cycleNodes=await page.locator('#genesisCycle .node').count();assert(cycleNodes===(DATA.payload.pipeline.steps.length),`${name}: cycle incomplet`);const observationCount=await page.locator('#trainingObservations .item').count();assert(observationCount===DATA.payload.training_field.observations.length,`${name}: observations incomplètes`);const gateCount=await page.locator('#publicationGates .item').count();assert(gateCount===DATA.payload.publication_gates.gates.length,`${name}: gates incomplets`);
