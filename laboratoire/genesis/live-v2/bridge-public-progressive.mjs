...
function scanSensitive(value, label = '$') {
  if (typeof value === 'string') {
    const patterns = [
      ...
      [/public\/live-source/i, 'private publication branch reference'],
      [/public\/live\/status\.env/i, 'private publication path reference'],
    ];
...
function validateAttestation(a) {
  ...
  eq(a.read_capability, 'READ_ONLY', '$.source_attestation.read_capability');
  eq(a.write_capability, 'NONE', '$.source_attestation.write_capability');
  eq(a.adapter_only, true, '$.source_attestation.adapter_only');

  if (a.validated_through === 'C060') {
    eq(a.source_state, 'C041_C060_COMPLETE_VALIDATED', '$.source_attestation.source_state');
    eq(a.c061_status, 'NOT_APPLICABLE', '$.source_attestation.c061_status');
...
export function buildProgressivePublicEnvelope(input, options = {}) {
  ...
  eq(input.bridge_input_version, '2.0.0', '$.bridge_input_version');
  eq(input.publication_intent, 'SERVER_SIDE_PUBLIC_READ_ONLY_PROGRESSIVE_BRIDGE', '$.publication_intent');
  ...
  if (ageSeconds > input.max_age_seconds) fail('Source progressive périmée: fail closed.');

  const a = input.source_attestation;
  const liveActive = input.live_active;
  const metrics = [
  ...
  const payload = {
    identity: {
      ...
      root_version: `GENESIS-003-${a.validated_through}-PUBLIC-PROGRESS-v2`,
    ...
    metrics,
    evidence: [
      { id: 'SERVER-SIDE-WHITELIST-V2', type: 'PUBLIC_ATTESTATION', status: 'VERIFIED_PUBLIC', public_ref: 'Whitelisted progressive Genesis status projection' },
    ],
    integrity: {
  ...
  const envelope = {
    ...
    mode: 'LIVE_READ_ONLY',
    publication_id: 'GENESIS-PUBLIC-READ-ONLY-PROGRESSIVE-0002',
  ...
  return { envelope, sourceAgeSeconds: ageSeconds };
}

async function main() {
  const inputPath = process.argv[2];
  if (!inputPath) fail('Usage: bridge-public-progressive.mjs <bridge-input.json>');
  const input = JSON.parse(await readFile(path.resolve(inputPath), 'utf8'));
  const result = buildProgressivePublicEnvelope(input);
  ...
  console.log(JSON.stringify({
    ...
    validated_through: result.envelope.payload.metrics.find((x) => x.id === 'genesis003-validated-through')?.value,
    public_live_active: result.envelope.payload.metrics.find((x) => x.id === 'public-live-active')?.value,
  }, null, 2));
}

if (process.argv[1] === fileURLToPath(import.meta.url)) {
  main().catch((error) => {
    console.error(`GENESIS_PROGRESSIVE_PUBLIC_READ_ONLY_BRIDGE_INVALID: ${error.message}`);
    process.exit(1);
  });
}
