...
try{
    ...
    $required=@(
      ...
      'lib/PathPolicy.ps1','lib/Audit.ps1','checksums.sha256'
    )
    ...
    $runtimeText=@('cli.ps1','Invoke-Inspect.ps1','lib/Common.ps1','lib/CanonicalJson.ps1','lib/PathPolicy.ps1','lib/Audit.ps1')|ForEach-Object{Get-Content -LiteralPath (Join-Path $root $_) -Raw -Encoding UTF8}|Out-String
    foreach($check in @(
    ...
    Add-LinuxIATestResult 'SUITE-001' ([int]$suite.expected_count -eq 21 -and @($suite.tests).Count -eq 21) 'CLI suite count is exact'
