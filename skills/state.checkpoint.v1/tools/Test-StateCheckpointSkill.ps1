...
try {
    ...
    Write-Host 'ANTMUX STATE CHECKPOINT SKILL VALIDATOR'
    ...
    $required=@('README.md','skill.yaml','instructions.md','prompts/checkpoint-operation.md','resources/checkpoint-rules.md','resources/error-codes.md','schemas/input.schema.json','schemas/output.schema.json','tests/manifest.tests.json','tests/permissions.tests.json','tests/contracts.tests.json','tools/Test-StateCheckpointSkill.ps1','tools/lib/Common.ps1','tools/lib/InputContract.ps1','tools/lib/OutputContract.ps1','tools/lib/Permissions.ps1','tools/lib/Integrity.ps1','checksums.sha256')
    foreach ($rel in $required) { Add-TestResult ('FILE-' + ($rel -replace '[^A-Za-z0-9]','_').ToUpperInvariant()) (Test-Path -LiteralPath (Join-Path $root $rel) -PathType Leaf) "Required file exists: $rel" }
    ...
    Add-TestResult 'SUITE-MANIFEST' ([int]$m.expected_count -eq 14 -and @($m.tests).Count -eq 14) 'Manifest suite count is exact'
    Add-TestResult 'SUITE-PERMISSIONS' ([int]$p.expected_count -eq 16 -and @($p.tests).Count -eq 16) 'Permission suite count is exact'
    Add-TestResult 'SUITE-CONTRACTS' ([int]$c.expected_count -eq 20 -and @($c.tests).Count -eq 20) 'Contract suite count is exact'
    ...
    Add-TestResult 'SCHEMA-001' ($inputSchemaId -eq 'antmux://schemas/state-checkpoint-input-v1') 'Input schema identifier is exact'
    Add-TestResult 'SCHEMA-002' ($outputSchemaId -eq 'antmux://schemas/state-checkpoint-output-v1') 'Output schema identifier is exact'
    ...
    $allow=@(Get-YamlList $manifest 'permissions.tools.allow');$deny=@(Get-YamlList $manifest 'permissions.tools.deny')
    ...
    foreach ($t in @($c.tests)) {
        $fixture=if([string]$t.subject -eq 'input'){New-InputFixture ([string]$t.case)}else{New-OutputFixture ([string]$t.case)}
        $actual=if([string]$t.subject -eq 'input'){Test-InputContract $fixture}else{Test-OutputContract $fixture}
        Add-TestResult ([string]$t.id) ($actual -eq [bool]$t.valid) ("Contract case: " + $t.case)
    }
